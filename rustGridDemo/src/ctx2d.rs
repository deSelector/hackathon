...
pub fn _draw_text(ctx: &CanvasRenderingContext2d, v: &str, x: f64, y: f64) {
    ctx.stroke_text(v, x, y).unwrap();
}
...
pub fn draw_text_aligned(
    ...
    match &*ctx.text_align() {
        "right" => ctx.stroke_text(v, x + width - PADDING, dy).unwrap(),
        "center" => ctx.stroke_text(v, x + width / 2.0, dy).unwrap(),
        _ => ctx.stroke_text(v, x + PADDING, dy).unwrap(),
    };
}

pub fn draw_horizontal_line(ctx: &CanvasRenderingContext2d, left: f64, right: f64, y: f64) {
    ctx.move_to(left, y);
    ctx.line_to(right, y);
}

pub fn draw_vertical_line(ctx: &CanvasRenderingContext2d, top: f64, bottom: f64, x: f64) {
    ctx.move_to(x, top);
    ctx.line_to(x, bottom);
}

pub fn set_stroke(ctx: &CanvasRenderingContext2d, color: &str) {
...
pub fn set_font(ctx: &CanvasRenderingContext2d, font: &str) {
    ctx.set_font(font);
}
